    ...
    def generate(self, context, beam_size=5, length_penalty=1.0):
        ...
        for step in range(self.max_length - 1):
            ...
            if step == 0:
                ...
                # Reshape sequence scores and log probs
                ...
                # Flatten scores for topk selection
                ...
                batch_pos = torch.arange(batch_size, device=device).unsqueeze(1) * num_beams  # [batch_size, 1]
                beam_indices = beam_indices + batch_pos  # [batch_size, beam_size]
                ...
                # Update scores
                sequence_scores = scores.view(-1, 1)  # [batch_size * beam_size, 1]
                
                # Gather context if needed
                if context.size(0) != sequences.size(0):
                    context = context.view(batch_size, num_beams, *context.shape[1:])
                    context = context[:, 0].repeat_interleave(beam_size, dim=0)
